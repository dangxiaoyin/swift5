...
// MARK: -  属性包装器
 ...
 上面例子以 private 的方式声明 number 变量，这使得 number 仅在 TwelveOrLess 的实现中使用。写在其他地方的代码通过使用 wrappedValue 的 getter 和 setter 来获取这个值，但不能直接使用 number。有关 private 的更多信息，请参考 访问控制
